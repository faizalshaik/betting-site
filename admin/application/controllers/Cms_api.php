...
class Cms_api extends CI_Controller
{
	...
	public function get_bets_list($week = 0, $repeat = 0, $ticketNo = 'null', $betId = 0, $betAbove = 0, $fromDt = 'null', $toDt = 'null', $betStatus = 'Any', $agentId = 0, $terminalId = 0, $optionId = 0)
	{
		$this->logonCheck();
		...
		foreach ($bets as $bet) {
			...
			if ($option != null) $optName = $option->name;
...
// 		if ($ticketNo != 'null') $condition['ticket_no'] = $ticketNo;
	...
	public function get_result_summary_by_agent($week = 0, $staffId = 0, $agentId = 0, $terminalId = 0)
	{
		...
		foreach ($summaries as $summary) {
			...
			if (isset($terminals[$terminal->Id]) == false)
				$rowTerminal = array('terminal' => $terminal->terminal_no, 'sales' => array(), 'payable' => array(), 'wins' => array());
			else $rowTerminal = $terminals[$terminal->Id];

			$payables = $rowTerminal['payable'];
	...
	public function get_void_bets_list($week = 0, $repeat = 0, $ticketNo = 'null', $amt = 0, $betId = 'null', $fromDt = 'null', $toDt = 'null', $betStatus = 0, $agentId = 0, $terminalId = 0, $optionId = 0)
	{
		$this->logonCheck();
		$no = $this->input->post("start");
		$length = $this->input->post("length");
